...
int vtkSOTResult::CreateAxes(double x, double y, double dz)
{
	...
	// Z Axis
	axesPoints->InsertNextPoint(x, y, 0.0);
	...
	// X Axis
	axesPoints->InsertNextPoint(x, y, 0.0);
	axesPoints->InsertNextPoint(x+ReferenceCirclesSize, y, 0.0);
	axesCells->InsertNextCell(2);
	...
	// Y Axis
	axesPoints->InsertNextPoint(x, y, 0.0);
	axesPoints->InsertNextPoint(x, y+ReferenceCirclesSize, 0.0);
	axesCells->InsertNextCell(2);
...
int vtkSOTResult::transformCoordinates()
{
	...
	{
		this->addToVerts( (*it).radius, (*it).distance, (*it).angle, pointCounter, 0, 0 );
		pointCounter++;
	}
...
//---------------------------------------------------------------------------------
int vtkSOTResult::transformComparasionCoordinates()
{
	vtkIdType pointCounter = 0;
	this->verts = vtkCellArray::New();
	this->points = vtkPoints::New();
		...
		it != this->internals->coordVector.end(); it++)
	{
		...
		this->addToVerts( (*it).radius, (*it).distance, (*it).angle, pointCounter, dx, dy );
...
//--------------------------------------------------------------------------------
int vtkSOTResult::addComparaisonGrids()
{
	int j;
	...
	for(int i=1; i<=num; i++)
	{
		//this->addInitialGrid(x+posx, y, 0, this->minDistance + ((i-1)*labelStep) );
		//this->AddGrid(x+posx, y, this->DistanceBetween/this->numberOfGrids, -1.111
			///*this->minDistance + (i*labelStep)*/ );
		textStream << (*it);
		textStream >> text;
		textStream.clear();
		it++;
													...
													vtkStdString compareLabel, bool showCompLabel)
{
	...
	textStream >> text;
	textStream.clear();
	//this->internals->split(text, textSplit, ".");

	//if(textSplit.size() > 0)
	//{
		//numberOfDigits = textSplit[0].size() + 2;
	//}
	...
	for(int i=0; i<=this->numberOfCircles; i++)
	{
		...
		//this->internals->split(text, textSplit, ".");
		//if(text.size() >= numberOfDigits)
		//{
			//text = text.substr(0, numberOfDigits);
		//}
		//else
		//{
			//if(text.find('.') == vtkStdString::npos)
			//{
				//text += ".";
			//}

			//for(size_t j=text.size(); j<numberOfDigits; j++)
			//{
				//text += "0";
			//}
		//}
                          ...
                          vtkInformationVector* outputVector)
{
  ...
  this->circlesOutput = vtkPolyData::SafeDownCast( outInfo->Get(vtkDataObject::DATA_OBJECT()));
	...
	{ // Break points into comparison sections
		this->transformComparasionCoordinates();
	}
	else
