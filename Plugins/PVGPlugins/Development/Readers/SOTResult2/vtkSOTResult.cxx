...
int vtkSOTResult::addToVerts(double radius, double distance,  
	...
	if(!this->ReverseRadius)
		{
		initialY = ( radius - this->minRadius ) * ReferenceCirclesSize/this->deltaRadius;
		}
	else
		{
	...
	double angl = (( (angle-this->minAngle) / this->deltaAngle ) * var) + ( 2.0 * (PI / 3.0));
	...
	if(this->EnableClip)
		{
		if(strcmp(this->ClipType, "Plane") == 0)
			{
			// Distance that we will allow points to be drawn within for clipping
			// Currently 2% or total distance
			double zFudgeDistance = (this->maxDistance - this->minDistance)*
				(this->DistanceBetween/this->deltaDistance)*0.02;

			// If clipping, make sure we only take points within zFudgeDistance
			if(-z > this->PlaneDistanceVariable-zFudgeDistance && -z < this->PlaneDistanceVariable+zFudgeDistance)
				{
				...
				{
				// Don't add to point counter
				return 0;
				}
			}
		else if(strcmp(this->ClipType, "Box") == 0)
			{
...
int vtkSOTResult::transformCoordinates()
	...
	for(vtkstd::vector< pointDetails >::iterator it = this->internals->coordVector.begin();
		it != this->internals->coordVector.end(); it++)
		{
		retVal = this->addToVerts( (*it).radius, (*it).distance, (*it).angle, pointCounter, 0, 0 );
...
int vtkSOTResult::transformComparisonCoordinates()
	...
	for(vtkstd::vector< pointDetails >::iterator it = this->internals->coordVector.begin();
		...
		retVal = this->addToVerts( (*it).radius, (*it).distance, (*it).angle, pointCounter, dx, dy );
	...
	if(this->numberOfGrids>0)
		...
		if(this->EnableClip)
			...
			if(strcmp(this->ClipType, "Plane") == 0)
				...
				double zLabelDbl = (this->PlaneDistanceVariable/this->DistanceBetween) * this->deltaDistance;
...
int vtkSOTResult::RequestData(vtkInformation* request,
	...
	if(!this->AlignCircles)
		...
		if(this->EnableClip)
			...
			if(strcmp(this->ClipType, "Plane") == 0)
				{
				this->addNormalViewGrids(0, 0, -this->PlaneDistanceVariable, "", false);
				this->CreateAxes(0, 0, -this->PlaneDistanceVariable);
				}
			else if(strcmp(this->ClipType, "Box") == 0)
				{
