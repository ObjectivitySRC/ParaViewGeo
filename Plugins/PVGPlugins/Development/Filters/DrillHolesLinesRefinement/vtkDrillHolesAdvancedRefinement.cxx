/*=========================================================================
  ...
  Module:    $ vtkDrillHolesAdvancedRefinementTo.cxx $
  ...
  =========================================================================*/
#include "vtkDrillHolesAdvancedRefinement.h"
...
vtkCxxRevisionMacro(vtkDrillHolesAdvancedRefinement, "$Revision: 0.1 $");
vtkStandardNewMacro(vtkDrillHolesAdvancedRefinement);

//----------------------------------------------------------------------------
vtkDrillHolesAdvancedRefinement::vtkDrillHolesAdvancedRefinement()
{
...
* Date:					                                                  *
***************************************************************************/
int vtkDrillHolesAdvancedRefinement::RequestDataObject(vtkInformation *,
...
* Date:					                                                  *
***************************************************************************/
float vtkDrillHolesAdvancedRefinement::DistancePointToLine(double P[3], double A[3], double B[3])
{
	float distance=-1000;
...
* Date:					                                                  *
***************************************************************************/
float vtkDrillHolesAdvancedRefinement::DistancePointToLine2(double point[3], int HoleId,vtkCellArray* CellsInput1,
...
* Date:					                                                  *
***************************************************************************/
void vtkDrillHolesAdvancedRefinement::SetRBlockInfo(vtkRectilinearGrid *grid,
												 int level,
												 int *ext,
												 int onFace[6],
												 int HoleId)
...
int vtkDrillHolesAdvancedRefinement::RequestInformation(
...
//----------------------------------------------------------------------------
int vtkDrillHolesAdvancedRefinement::FillInputPortInformation ( int vtkNotUsed(port), vtkInformation* info )
  {
...
//----------------------------------------------------------------------------
int vtkDrillHolesAdvancedRefinement::FillOutputPortInformation(int vtkNotUsed(port), vtkInformation* info)
{
  info->Set(vtkDataObject::DATA_TYPE_NAME(), "vtkCompositeDataSet");
  return 1;
}
 
int vtkDrillHolesAdvancedRefinement::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  { 
	 //make the interpolation of the drillHoles
     vtkInterpolateHolesFilter *input1= vtkInterpolateHolesFilter::New();
     input1->SetInput(input);
     input1->SetTypeOfInterpolation(this->TypeOfInterpolation);
     input1->Update();


	 this->CellsInput = vtkCellArray::SafeDownCast(input1->GetOutput()->GetLines());
	 this->PointCoordsInput = input1->GetOutput()->GetPoints()->GetData();	
	 vtkCellData* p= input1->GetOutput()->GetCellData();
...
/*
  vtkThreshold *connect= vtkThreshold::New();
  //vtkDataObject *doOutput1=outInfo->Get(vtkDataObject::DATA_OBJECT());
  vtkDataSet *connect1=vtkDataSet::SafeDownCast(doOutput);
  connect1->CopyStructure(output);
 // connect1->GetData()->PassData ( output->GetPointData() );
 // connect1->GetCellData()->PassData ( output->GetCellData() );

  connect->SetInput(connect1);
  connect->SetInputArrayToProcess(0, 0,0,vtkDataObject::FIELD_ASSOCIATION_POINTS, "DistanceToHole");
  connect->ThresholdBetween(0,this->Distance);
  connect->Update();
...
* Date:					                                                  *
***************************************************************************/
int vtkDrillHolesAdvancedRefinement::LineIntersectBoxTest(double x0, double y0, double z0,
...
* Date:					                                                  *
***************************************************************************/
int vtkDrillHolesAdvancedRefinement::subdivideTest(double bds[6], int level) 
{
...
* Date:					                                                  *
***************************************************************************/
void vtkDrillHolesAdvancedRefinement::RefineBlock(int &blockId,
									  int level,
                                      ...
                                      int onFace[6],int id)
{
  ...
  if (x3-x2-x1+x0 > 2)
   ...
   // 3D
   int res=0;
   res= subdivideTest( bds, level);
   if( (this->TypeOfRefinement==0) &&  (res>0) )
	   subdivide=1;
   if( (this->TypeOfRefinement==1) &&  (res==1) )
       subdivide=1;
 ...
 if (level < this->MinimumLevel)
  ...
  //int id1=0;
  if((this->TypeOfRefinement==0) && (subdivide==1) )
	  id=res;
   if(subdivide )
      ...
      subOnFace[5]=0;
      this->RefineBlock(blockId, level, output, x0,x1,y0,y1,z0,z1,subOnFace,id);
     
	  subOnFace[0]=0;
      subOnFace[1]=onFace[1];
      this->RefineBlock(blockId, level, output, x2,x3,y0,y1,z0,z1,subOnFace,id);
     
	  subOnFace[0]=onFace[0];
      subOnFace[1]=0;
      subOnFace[2]=0;
      subOnFace[3]=onFace[3];
      this->RefineBlock(blockId, level, output, x0,x1,y2,y3,z0,z1,subOnFace,id);
      
	  subOnFace[0]=0;
      subOnFace[1]=onFace[1];
      this->RefineBlock(blockId, level, output, x2,x3,y2,y3,z0,z1,subOnFace,id);
      ...
      this->RefineBlock(blockId, level, output, x0,x1,y0,y1,z2,z3,subOnFace,id);
    
	  subOnFace[0]=0;
      subOnFace[1]=onFace[1];
      this->RefineBlock(blockId, level, output, x2,x3,y0,y1,z2,z3,subOnFace,id);
     
	  subOnFace[0]=onFace[0];
      subOnFace[1]=0;
      subOnFace[2]=0;
      subOnFace[3]=onFace[3];
      this->RefineBlock(blockId, level, output, x0,x1,y2,y3,z2,z3,subOnFace,id);
    
	  subOnFace[0]=0;
      subOnFace[1]=onFace[1];
      this->RefineBlock(blockId, level, output, x2,x3,y2,y3,z2,z3,subOnFace,id);
      }
    else 
      {
...
* Date:					                                                  *
***************************************************************************/
void vtkDrillHolesAdvancedRefinement::CellExtentToBounds(int level,
                                                int ext[6],
                                                double bds[6])
{
  int spacingFactor = 1;
...
* Date:					                                                  *
***************************************************************************/
unsigned int vtkDrillHolesAdvancedRefinement::AppedDataSetToLevel(
...
void vtkDrillHolesAdvancedRefinement::PrintSelf(ostream& os, vtkIndent indent)
{
