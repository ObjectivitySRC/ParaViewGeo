#include "pqDataMineWireFrameReader.h"
...
pqDataMineWireFrameReader::pqDataMineWireFrameReader(pqProxy* pxy, QWidget* p) :
  pqLoadedFormObjectPanel(":/StableReaders/DataMineReader/pqDMWireFrameReader.ui", pxy, p)
{      
...
pqDataMineWireFrameReader::~pqDataMineWireFrameReader()
{
...
void pqDataMineWireFrameReader::pointFileLoad()
{
...
void pqDataMineWireFrameReader::topoFileLoad()
{
...
bool pqDataMineWireFrameReader::FileLoad( QString &name, QString &filterText, QLineEdit *lineEdit )
{
...
void pqDataMineWireFrameReader::accept()
{
	//we only want to push the status of all the model items at accept time
	//rather than when each item is checked/unchecked.

	//also the two checkboxes are combined together to get the StringValue
	// 0 = no checked
	// 1 = first checkbox checked
	// 2 = second checkbox checked
...
void pqDataMineWireFrameReader::reset()
{
  // reset widgets controlled by the parent class
  pqLoadedFormObjectPanel::reset();
}

void pqDataMineWireFrameReader::linkServerManagerProperties()
{
...
void pqDataMineWireFrameReader::updateProperties()
{
	vtkSMStringVectorProperty *stringVector = vtkSMStringVectorProperty::SafeDownCast( this->proxy()->GetProperty("CellArrayStatus"));
