...
pqApplySchedule::pqApplySchedule(pqProxy* pxy, QWidget* p) :
  pqLoadedFormObjectPanel(":/StableFilters/ApplySchedule/pqApplySchedule.ui", pxy, p)
  {
		...
		this->DurationPropertyName = this->findChild<QComboBox*>("DurationPropertyName");
		this->DurationPropertyName->setObjectName("DurationPropertyName");
...
//--------------------------------------------------------------------------------
pqApplySchedule::~pqApplySchedule()
{
}
...
//--------------------------------------------------------------------------------
void pqApplySchedule::linkServerManagerProperties()
{
...
//--------------------------------------------------------------------------------
void pqApplySchedule::clearTimeColumn(int id)
{
...
//--------------------------------------------------------------------------------
void pqApplySchedule::onTableSelectionChanged(QTableWidgetItem * item)
{
	this->durationRow = this->DurationPropertyName->currentIndex();
	bool start = false;
	...
	if(!loading)
	{
		if(checked)
		{
			...
			// if any item in the second row is selected, on item in the duration
			// line must be selected
			if(item->column() == 1)
			{
				bool addDuration = true;
				// if duration is already selected, we don't have to select it again
				for(int i=0 ; i<3; i++)
				{
					if( this->table->item(this->durationRow, i)->checkState() == Qt::Checked )
					{
						addDuration = false;
					}
				}
				if(addDuration)
					this->table->item(this->durationRow, 0)->setCheckState(Qt::Checked);
			}
		...
		{
			if(item->row() == this->durationRow)
			{
				bool removeStart = true; // must be renamed removeTime

				// if duration item is unchecked, all items in time column
...
//--------------------------------------------------------------------------------
void pqApplySchedule::updateTable()
{
...
//--------------------------------------------------------------------------------
void pqApplySchedule::restoreRowState(QString &value)
{
...
//--------------------------------------------------------------------------------
void pqApplySchedule::onStateReloaded()
{

	vtkSMSourceProxy* sp = vtkSMSourceProxy::SafeDownCast(this->proxy());
	//vtkErrorMacro(""<<sp->GetXMLLabel());
	vtkSMStringVectorProperty *durationPropVector = 
		vtkSMStringVectorProperty::SafeDownCast(sp->GetProperty("DurationPropertyName"));
	QString duration = durationPropVector->GetElement(0);

	if(!durationPropVector)
	{
		qDebug() << "duration properties not found in the xml file";
	}

	for(int i=0; i<this->DurationPropertyName->count(); i++)
	{
		if(this->DurationPropertyName->itemText(i) == duration)
		{
			this->DurationPropertyName->setCurrentIndex(i);
			break;
		}
	}
...
//--------------------------------------------------------------------------------
void pqApplySchedule::onBrowse()
{
  QString filters;
  filters += "Schedule file (*.exf)";
	filters += ";;Sot schedule file (*.csv)";
  filters += ";;All files (*)";
...
//--------------------------------------------------------------------------------
void pqApplySchedule::onFileNameChanged( )
{
	...
	QObject::disconnect(this->DurationPropertyName, SIGNAL( currentIndexChanged(int) ),
	...
	foreach(QString prop, list)
	{
		...
		if( list2[i][0].isDigit() || atof(list2[i].toAscii().data()) )//ok || wanted.contains(prop.toLower()))
		{
			...
			this->DurationPropertyName->addItem(prop);
		}
		i++;
	}
	...
	QObject::connect(this->DurationPropertyName, SIGNAL( currentIndexChanged(int) ),
		this, SLOT( clearTimeColumn(int)) );
