/*=========================================================================
  ...
  Module:    $RCSfile: vtkExtractUnstrucGrid.cxx,v $

	By:				 Matthew Livingstone
=========================================================================*/
#include "vtkExtractUnstrucGrid.h"
...
class vtkExtractUnstrucGrid::vtkSet : public vtkstd::set<unsigned int>
{
};

vtkStandardNewMacro(vtkExtractUnstrucGrid);
vtkCxxRevisionMacro(vtkExtractUnstrucGrid, "$Revision: 1.4 $");
vtkInformationKeyMacro(vtkExtractUnstrucGrid, DONT_PRUNE, Integer);
//----------------------------------------------------------------------------
vtkExtractUnstrucGrid::vtkExtractUnstrucGrid()
{
  this->Indices = new vtkExtractUnstrucGrid::vtkSet();
  this->PruneOutput = 1;
}
...
//----------------------------------------------------------------------------
void vtkExtractUnstrucGrid::AddIndex(unsigned int index)
{
...
//----------------------------------------------------------------------------
void vtkExtractUnstrucGrid::RemoveIndex(unsigned int index)
{
...
//----------------------------------------------------------------------------
void vtkExtractUnstrucGrid::RemoveAllIndices()
{
  this->Indices->clear();
  this->Modified();
}
...
//----------------------------------------------------------------------------
void vtkExtractUnstrucGrid::CopySubTree(vtkCompositeDataIterator* loc, 
...
//----------------------------------------------------------------------------
int vtkExtractUnstrucGrid::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  vtkMultiBlockDataSet *input =  vtkMultiBlockDataSet::SafeDownCast (
	...
	vtkUnstructuredGrid *unstrucOutput = vtkUnstructuredGrid::SafeDownCast (
  ...
  if (this->PruneOutput)
	...
	if(selectedItems->GetNumberOfBlocks() <= 0)
	{
		...
		unstrucOutput->CopyStructure(temp);
		temp->Delete();
		selectedItems->Delete();
		return 1;
	}

	// Final UnstructuredGrid output object
	vtkAppendFilter *unstrucGroup = vtkAppendFilter::New();

	// Append all blocks to the UnstructuredGrid output
  ...
  iter2->TraverseSubTreeOn();
	while( !iter2->IsDoneWithTraversal() )
		{
		unstrucGroup->AddInput(vtkUnstructuredGrid::SafeDownCast( iter2->GetCurrentDataObject() ) );
		iter2->GoToNextItem();
		}
	iter2->Delete();
	...
	unstrucOutput->ShallowCopy(unstrucGroup->GetOutput());
...
//----------------------------------------------------------------------------
bool vtkExtractUnstrucGrid::Prune(vtkDataObject* branch)
{
...
//----------------------------------------------------------------------------
bool vtkExtractUnstrucGrid::Prune(vtkMultiPieceDataSet* mpiece)
{
...
//----------------------------------------------------------------------------
bool vtkExtractUnstrucGrid::Prune(vtkMultiBlockDataSet* mblock)
{
...
//----------------------------------------------------------------------------
void vtkExtractUnstrucGrid::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
  os << indent << "PruneOutput: " << this->PruneOutput << endl;
}

int vtkExtractUnstrucGrid::FillInputPortInformation(int, vtkInformation *info)
{
