  ...
  vtkInformationVector *outputVector)
{
 ...
 // cout << "Points set nbpt=" << numCells << endl;
  ...
  if(WholeOutput==1)
  {
	...
	if(processid== 0)//that mean the process is the root
	{
    ...
    outInfo->Get(vtkDataObject::DATA_OBJECT()));
	  ...
	  copy.TakeReference(output->NewInstance());//important when to collect a data to create 
	...
	if(processid== 0)//that mean the process is the root
	{
		output->ShallowCopy(reduceFilter->GetOutput()); // with that my data is still on different process
		
		/*we don't need
		//now i want to collect the data and have it on the main process
		vtkCollectPolyData *collect= vtkCollectPolyData::New();
		collect->SetController(this->Controller);
		collect->SetInput(output);

		output->ShallowCopy( collect->GetOutput() );
		*/
	}
