/*=========================================================================
  ...
  Module:    $RCSfile: vtkSurfaceReconstructor.cxx,v $
...
=========================================================================*/
#include "vtkSurfaceReconstructor.h"
...
vtkCxxRevisionMacro(vtkSurfaceReconstructor, "$Revision: 0.1 $");
vtkStandardNewMacro(vtkSurfaceReconstructor);

vtkSurfaceReconstructor::vtkSurfaceReconstructor()
{
...
//----------------------------------------------------------------------------
int vtkSurfaceReconstructor::FillInputPortInformation(
...
int vtkSurfaceReconstructor::RequestInformation (
...
//-----------------------------------------------------------------------------
int vtkSurfaceReconstructor::RequestData(
  vtkInformation* vtkNotUsed( request ),
  vtkInformationVector** inputVector,
  vtkInformationVector* outputVector)
{
  ...
  vtkUnstructuredGrid *actualOutput = vtkUnstructuredGrid::New();
  
  vtkPolyData *finalOutput = vtkPolyData::SafeDownCast(
  ...
  if(orientationPropagation) 
    ...
    while(nearby->GetNumberOfIds()>0)
      ...
      if(connectedVisited == cheapestNearby)
        {
        vtkErrorMacro (<< "Internal error in vtkSurfaceReconstructor");
        return 0;
        }
  ...
  //CCAMERON BEGIN check for x,y,z ranges that are 0. 
  double volume = 0;
  int isZero[3];
  for (i = 0; i < 6; i+=2)
    {
  ...
  if (isZero[0] + isZero[1] + isZero[2] > 1) // 1D surface impossible
    {
    vtkErrorMacro(<< "1 dimensional surface cannot be constructed")
    return 1;
    }
  else if(isZero[0] == 1) //2D surface no x dimension
    {
    volume = static_cast<double> (bounds[3] - bounds[2]) * (bounds[5] - bounds[4]);
    }
  else if(isZero[1] == 1) //2D surface no y dimension
    {
    volume = static_cast<double> (bounds[2] - bounds[1]) * (bounds[5] - bounds[4]);
    }
  else if(isZero[2] == 1) //2D surface no z dimension
    {
    volume = static_cast<double> (bounds[3] - bounds[2]) * (bounds[1] - bounds[0]);
    }
  else //3D surface
    {
    volume = static_cast<double> (bounds[1]-bounds[0]) * (bounds[3]-bounds[2]) * (bounds[5]-bounds[4]);
    }
  //CCAMERON END check for x,y,z ranges that are zero

  // spacing guessed as cube root of (volume divided by number of points)
  this->SampleSpacing = pow(volume /
                            static_cast<double>(COUNT),
                            static_cast<double>(1.0/3.0));

  if (this->Resolution == 2)
    {
    this->SampleSpacing /= pow(2, 0.33333);
    }
  else if(this->Resolution == 4)
    {
    this->SampleSpacing /= pow(4, 0.33333);
    }
  else if(this->Resolution == 8)
    {
    this->SampleSpacing /= 2;
    }
  else if (this->Resolution == 16)
    {
    this->SampleSpacing /= pow(16, 0.33333);
    }
  else if (this->Resolution == 32)
    {
    this->SampleSpacing /= pow(32, 0.33333);
    }
  else if (this->Resolution == 64)
    {
    this->SampleSpacing /= pow(64, 0.33333);
    }
  ...
  for(i=0;i<3;i++)
    {
    bounds[i*2]-=this->SampleSpacing*2; //CCAMERON was *2
    bounds[i*2+1]+=this->SampleSpacing*2; //CCAMERON was *2
    }
  ...
  //CCAMERON BEGIN set whole extents, extents, update extents, dimensions, spacing, origin, allocate scalars
  ...
  output->SetDimensions(dim[0], dim[1], dim[2]); //CCAMERON original filter forgot dimensions?
  ...
  //CCAMERON END reorder extents

  
  
  //CCAMERON BEGIN creating float array without using GetScalars() method
  // also need to set the name of the array, or it won't show up in Object Inspector
  vtkFloatArray *newScalars = vtkFloatArray::New();
  newScalars->SetNumberOfComponents(1);
  newScalars->SetNumberOfTuples(output->GetNumberOfPoints());
  newScalars->SetName("Distance");
  //CCAMERON END
  ...
  for(z=0;z<dim[2];z++)
    ...
    for(y=0;y<dim[1];y++)
      ...
      for(x=0;x<dim[0];x++)
        ...
        //CCAMERON BEGIN Hoppe's original code checks to see if point is too far from data point
        int pointId = locator2->FindClosestPoint(point);
        double foundPoint[3];
        input->GetPoint(pointId, foundPoint);
        double distance = vtkMath::Distance2BetweenPoints(foundPoint, point);
        ...
        if (distance > this->SampleSpacing * 2.0)
          {
          if (probeValue > 0)
            {
            newScalars->SetValue(offset, 1e10f); //undefined
            }
          else
            {
            newScalars->SetValue(offset, 1e10f); //undefined
  ...
  locator2->Delete();
  //CCAMERON BEGIN now you have to add the properties to the output
  ...
  //CCAMERON now we want to threshold out the undefined values, and return the contour at 0
  
  vtkThreshold *thresh = vtkThreshold::New();
  thresh->SetInput(output);
  thresh->AllScalarsOn();
  thresh->SetInputArrayToProcess(0, 0,0,vtkDataObject::FIELD_ASSOCIATION_POINTS, "Distance");
  thresh->ThresholdByLower(100);
  thresh->Update();

  actualOutput->ShallowCopy(thresh->GetOutput());

  thresh->Delete();
  output->Delete();

  vtkContourFilter *contour = vtkContourFilter::New();
  contour->SetInput(actualOutput);
  contour->SetComputeNormals(true);
  contour->SetInputArrayToProcess(0, 0,0,vtkDataObject::FIELD_ASSOCIATION_POINTS, "Distance");

  contour->SetNumberOfContours(1);
  contour->SetValue(0, 0);
  contour->Update();

  finalOutput->ShallowCopy(contour->GetOutput());
...
void vtkSurfaceReconstructor::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os,indent);
  
  os << indent << "Neighborhood Size:" << this->NeighborhoodSize << "\n";
  os << indent << "Resolution:" << this->Resolution << "\n";
  os << indent << "SampleSpacing:" << this->SampleSpacing << "\n";
}
