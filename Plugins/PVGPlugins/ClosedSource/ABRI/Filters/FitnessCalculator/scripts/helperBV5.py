...
class Helper(object):
  ...
  #####################################################################################
  def __init__(input, regularB, kdtree, result, xmin, xmax, ymin, ymax, zmin, zmax, dx, dy, dz, pointTable, d1, d2, d3, geoTechArray, AccessPos , densityArray):
    self.input = input
    self.xmin = xmin
    self.xmax = xmax
    self.ymin = ymin
  ...
  def computeFitness(self, iD):
    ...
    for point in neighbors:
      if(point[1] > 8):
        average += ((self.shared.getPropertyValue(Ni,point[0])*99.0) + (self.shared.getPropertyValue(Cu,point[0])*66.0) + (self.shared.getPropertyValue(Co,point[0])*2.2)) *self.shared.getPropertyValue(self.densityArray,iD)
