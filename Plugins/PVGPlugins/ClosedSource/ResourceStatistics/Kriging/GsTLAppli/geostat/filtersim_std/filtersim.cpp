...
/*
 * function to create a SGEMS interface for Filtersim_Std
 */
Named_interface* Filtersim_Std::create_new_interface( std::string& ) 
{
	return new Filtersim_Std;
}


/*
 * constructor function for Filtersim_Std
 */
Filtersim_Std::Filtersim_Std() 
{
	window_geom_ = 0;
	simul_grid_ = 0;
 ...
 */
bool Filtersim_Std::initialize( const Parameters_handler* parameters,
 ...
 */
int Filtersim_Std::execute( GsTL_project* proj ) 
{
    std::cout << "\n\nRunning algo Filtersim_Std... \n"; 
	...
	SmartPtr<Progress_notifier> progress_notifier = 
		        utils::create_notifier( "Running Filtersim_Std", total_steps, frequency );
 ...
 */
void Filtersim_Std::get_debug_level( const Parameters_handler* parameters )
{
 ...
 */
bool Filtersim_Std::get_marginal_data( const Parameters_handler* parameters,
                                  Error_messages_handler* error_mesgs )
{
	nb_facies_ = 1;
	...
	int nb_facies = (is_categorical_==1 ? nb_facies_ : 1 );
 ...
 */
bool Filtersim_Std::get_servo_system( const Parameters_handler* parameters,
                                  Error_messages_handler* error_mesgs )
{
 ...
 */
bool Filtersim_Std::get_marginal_cdf(std::string marginal_cpdf_str, 
                                 Error_messages_handler* error_mesgs)
{
 ...
 */
bool Filtersim_Std::get_simul_grid( const Parameters_handler* parameters,
		    Error_messages_handler* error_mesgs )
{
 ...
 */
bool Filtersim_Std::get_training_image( const Parameters_handler* parameters,
		    Error_messages_handler* error_mesgs )
{
 ...
 */
bool Filtersim_Std::get_hard_data( const Parameters_handler* parameters,
		                        Error_messages_handler* error_mesgs )
{
    ...
    if( !harddata_grid_name.empty() ) 
    {
        ...
        if( !ok ) 
        {
            error_mesgs->report( "Hard_Data", "Not a valid hard data grid" );
            return false;
        }
 ...
 */
bool Filtersim_Std::get_soft_data( const Parameters_handler* parameters,
                              Error_messages_handler* error_mesgs )
{
    ...
    if( use_soft_field_==1 )
    {
        ...
        if (prop_names.size() != nb_facies_)
        {
            if ( nb_facies_ == 1 )
                ...
                error_mesgs->report( "SoftData_properties", "Supply one soft data for continuous variable" );
 ...
 */
bool Filtersim_Std::get_region_data( const Parameters_handler* parameters,
                                     Error_messages_handler* error_mesgs )
{	
 ...
 */
bool Filtersim_Std::get_TI_scan_template(const Parameters_handler* parameters, 
                                     Error_messages_handler* error_mesgs)
{
 ...
 */
bool Filtersim_Std::get_patch_template(const Parameters_handler* parameters, 
                                   Error_messages_handler* error_mesgs)
{
	std::vector<int> template_window;
 ...
 */
void Filtersim_Std::get_nb_bins(const Parameters_handler* parameters)
{
 ...
 */
bool Filtersim_Std::get_cmin(const Parameters_handler* parameters, Error_messages_handler* error_mesgs)
{
 ...
 */
bool Filtersim_Std::get_data_weight(const Parameters_handler* parameters, Error_messages_handler* error_mesgs)
{
 ...
 */
bool Filtersim_Std::calculate_max_mgrid( Error_messages_handler* error_mesgs )
{
    ...
    */
    ...
    {
        cout<<"\nWarning: # of multi-grids is too large and is reset as " << nb_allowed << " !\n";
    ...
    if ( nb_multigrids_ <= 1 )
    {
        ...
        error_mesgs->report( "Nb_Multigrids_ADVANCED", "Search template is too large to use at least 2 multiple grids" );
        return false;
    }
 ...
 */
bool Filtersim_Std::get_filters(const Parameters_handler* parameters, 
                                Error_messages_handler* error_mesgs)
{
    // get filter type
 ...
 */
void Filtersim_Std::create_default_filter()
{
    // half template size
	int half_nxdt = (nxdt_-1)/2;  
	int half_nydt = (nydt_-1)/2;  
 ...
 */
bool Filtersim_Std::create_user_define_filter(const Parameters_handler* parameters, 
                                          Error_messages_handler* error_mesgs)
{
    ...
    if ( !valid_filter_file( half_nxdt, half_nydt, half_nzdt, filter_file_name ) )
    {
 ...
 */
bool Filtersim_Std::valid_filter_file( int nxdt, int nydt, int nzdt, string filter_file_name )
{   // nxdt, nydt, nzdt specify the half size of the searching template
    ...
    while( getline(infile,line) )
    {
        ...
        if ( str.size() == 2 )
        {
            ...
            if ( cur_filter >= total_nfilter)   return false;
            if ( total_weight_num == 0 )    return false;   // no weight specified
            if ( !is_number( str[1] ) )   return false;
 ...
 */
void Filtersim_Std::print_parameters()
{
 ...
 */
void Filtersim_Std::pre_progress_notifer(int& total_steps, int& frequency)
{
	float slow_factor=0;
 ...
 */
void Filtersim_Std::init_random_path() 
{
 ...
 */
void Filtersim_Std::set_region_indicator()
{
    nb_region_node_ = 0;
 ...
 */
void Filtersim_Std::get_filter_score_weight()
{
                                             ...
                                             GsTLGridProperty* prop, int nreal )
{
    ...
    int start_score_sim = -1;    // the starting coarse grid to be filled in using dual template
    int finest_grid_nb=1;     // mark the finest multiple grid nb: 1 for pixel-based distance; 0 for score-based distance  

    if ( use_score_dist_ )    
    {
        start_score_sim = 1;
        //finest_grid_nb = 0;     //un-comment this line to run simulation twice on the finest grid
    }
    ...
    for( int ncoarse = nb_multigrids_ ; ncoarse >= finest_grid_nb ; ncoarse-- ) 
    {
        ...
        */
        ...
        if ( is_categorical_ )      // for categorical variables
        {
            ...
            if ( is_cross_partition_ == 1 ) // categorical + use cross partition
            {
                if ( nreal == 1 && first_run )
                {
                ...
                if ( ncoarse>start_score_sim )   // use pixel-based distance calculation
                {
            ...
            else    // categorical + partition with k-mean
            {
                if ( nreal == 1 && first_run )
                {
                                                                     ...
                                                                     Pattern_Node_Map& mapping, int ncoarse )
{
    ...
    Grid_template* dual_window_geom = my_filters_->get_dual_window_geometry( ncoarse, (int)((nxdt_-1)/2), 
                                                                                                                (int)((nydt_-1)/2),  (int)((nzdt_-1)/2) );
    ...
    if ( use_region_==1 )
        ...
        patcher = new Patch_Helper( simul_grid_, harddata_property_!=0 );
    //*/
    //patcher = new Patch_Helper( prop );

    // set up a dual pattern paster to control the actual dual template pasting
    Pattern_Paster dual_paster(simul_grid_, dual_neighbors,  training_image_, dual_TI_neighbors, 
                                                 patcher, 2*patch_nxdt_-1, 2*patch_nydt_-1, 2*patch_nzdt_-1 );
                                                 //patcher, patch_nxdt_, patch_nydt_, patch_nzdt_ );

    // paste the patterns exactly along the random path with the dual_paster
    for (int i=0; i<mapping.total_mapping_pair(); i++)
        dual_paster( mapping.get_node_id(i), mapping.get_pattern_id(i) );
...
void 
Filtersim_Std::create_prototypelist( vector<CPrototype>& proto_list,
...
/*
 * function to create the SGEMS plugin dll
 */
//GEOSTAT_PLUGIN(Filtersim_Std);
