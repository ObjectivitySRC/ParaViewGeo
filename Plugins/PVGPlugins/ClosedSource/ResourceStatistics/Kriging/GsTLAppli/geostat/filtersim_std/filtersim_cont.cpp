...
#include "filtersim_cont.h"
#include "filtersim_tau_updating.h"
#include "filtersim_servo_system.h"
#include "sequential_patch_simulation.h"

/*
 * function to create a SGEMS interface for Filtersim_Cont
 */
Named_interface* Filtersim_Cont::create_new_interface( std::string& ) 
{
	return new Filtersim_Cont;
}


/*
 * constructor function for Filtersim_Cont
 */
Filtersim_Cont::Filtersim_Cont() 
{
	window_geom_ = 0;
	simul_grid_ = 0;
 ...
 */
bool Filtersim_Cont::initialize( const Parameters_handler* parameters,
 ...
 */
int Filtersim_Cont::execute( GsTL_project* proj ) 
{
    std::cout << "\n\nRunning algo Filtersim_Cont... \n"; 
	...
	SmartPtr<Progress_notifier> progress_notifier = 
		        utils::create_notifier( "Running Filtersim_Cont", total_steps, frequency );
 ...
 */
void Filtersim_Cont::get_debug_level( const Parameters_handler* parameters )
{
 ...
 */
bool Filtersim_Cont::get_marginal_data( const Parameters_handler* parameters,
                                  Error_messages_handler* error_mesgs )
{
	nb_facies_ = 1;
    ...
    // continuous variable
    /* // the target mean should be removed later since there is no more servosystem
    std::string marginal_ave_str = parameters->value( "Marginal_Ave.value" );
    if ( marginal_ave_str.empty() )
    {
        error_mesgs->report( "Marginal_Ave", "No target mean specified" );
        return false;
    }
    
    target_mean_ = String_Op::to_number<float>(parameters->value( "Marginal_Ave.value" ));
    */
 ...
 */
bool Filtersim_Cont::get_servo_system( const Parameters_handler* parameters,
                                  Error_messages_handler* error_mesgs )
{
    /*
 ...
 */
bool Filtersim_Cont::get_simul_grid( const Parameters_handler* parameters,
		    Error_messages_handler* error_mesgs )
{
 ...
 */
bool Filtersim_Cont::get_training_image( const Parameters_handler* parameters,
		    Error_messages_handler* error_mesgs )
{
	...
	if( !training_image_name_.empty() ) 
	{
        ...
        if ( transcon_data_==1)
        {
            // create a cdf from training image
            GsTLGridProperty* prop =training_image_->select_property( training_property_name_ );
            vector<float> TI_data;
            get_property_vector( prop, TI_data );

            build_cdf( TI_data.begin(), TI_data.end(), target_cdf_, includes_max_ );
            z_min_=*( target_cdf_.z_begin() );
            z_max_=*( target_cdf_.z_end()-1 );
        }
 ...
 */
bool Filtersim_Cont::get_hard_data( const Parameters_handler* parameters,
		                        Error_messages_handler* error_mesgs )
{
 ...
 */
bool Filtersim_Cont::get_soft_data( const Parameters_handler* parameters,
                              Error_messages_handler* error_mesgs )
{
    ...
    {
        softdata_property_names_ = parameters->value( "Soft_Data.value" );
        GsTLGridProperty* prop = simul_grid_->property( softdata_property_names_ );

        if( !prop ) 
        {
            error_mesgs->report( "Soft_Data", "The given soft data does not exist" );
            return false;
        }
 ...
 */
bool Filtersim_Cont::get_region_data( const Parameters_handler* parameters,
                                     Error_messages_handler* error_mesgs )
{	
 ...
 */
bool Filtersim_Cont::get_TI_scan_template(const Parameters_handler* parameters, 
                                     Error_messages_handler* error_mesgs)
{
 ...
 */
bool Filtersim_Cont::get_patch_template(const Parameters_handler* parameters, 
                                   Error_messages_handler* error_mesgs)
{
	std::vector<int> template_window;
 ...
 */
void Filtersim_Cont::get_nb_bins(const Parameters_handler* parameters)
{
 ...
 */
bool Filtersim_Cont::get_cmin(const Parameters_handler* parameters, Error_messages_handler* error_mesgs)
{
 ...
 */
bool Filtersim_Cont::get_data_weight(const Parameters_handler* parameters, Error_messages_handler* error_mesgs)
{
 ...
 */
bool Filtersim_Cont::calculate_max_mgrid( Error_messages_handler* error_mesgs )
{
    ...
    */
    ...
    if ( nb_multigrids_ == 1 && transcon_data_ == 1)
    {
        transcon_data_ = 0;
        GsTLcout<<"\nWarning: cannot transform result on 2nd grid because only 1 grid is actually used!"  << gstlIO::end;
    }
 ...
 */
bool Filtersim_Cont::get_filters(const Parameters_handler* parameters, 
                                Error_messages_handler* error_mesgs)
{
    // get filter type
 ...
 */
void Filtersim_Cont::create_default_filter()
{
    // half template size
	int half_nxdt = (nxdt_-1)/2;  
	int half_nydt = (nydt_-1)/2;  
 ...
 */
bool Filtersim_Cont::create_user_define_filter(const Parameters_handler* parameters, 
                                          Error_messages_handler* error_mesgs)
{
 ...
 */
bool Filtersim_Cont::valid_filter_file( int nxdt, int nydt, int nzdt, string filter_file_name,
                                                        Error_messages_handler* error_mesgs)
 ...
 */
void Filtersim_Cont::print_parameters()
{
 ...
 */
void Filtersim_Cont::pre_progress_notifer(int& total_steps, int& frequency)
{
	float slow_factor=0;
 ...
 */
void Filtersim_Cont::init_random_path() 
{
 ...
 */
void Filtersim_Cont::copy_pre_simulation_data()
{
 ...
 */
void Filtersim_Cont::set_region_indicator()
{
    nb_region_node_ = 0;
 ...
 * this program abort
 */
void Filtersim_Cont::clean( GsTLGridProperty* prop ) 
{
	if( prop ) 
 ...
 */
void Filtersim_Cont::get_filter_score_weight()
{
 ...
 */
bool Filtersim_Cont::simulate_one_realization( SmartPtr<Progress_notifier>& progress_notifier, 
                                             GsTLGridProperty* prop, int nreal )
{
    ...
    for( int ncoarse = nb_multigrids_ ; ncoarse >= finest_grid_nb ; ncoarse-- ) 
    {
        ...
        //only create score map for the first realization during the normal runs
        if ( nreal == 1 && first_run )  
        {
        ...
        */
        ...
        // same pattern paster for all continuous simulation situations
        Pattern_Paster_Continuous pattern_paster( simul_grid_, neighbors,  training_image_, TI_neighbors, 
                                                                          patcher, patch_nxdt_, patch_nydt_, patch_nzdt_ );

        if ( is_cross_partition_ == 1 ) // continuous + cross partition
        {
            if ( nreal == 1 && first_run )
            {
                create_prototypelist( proto_list3_, nreal, ncoarse, 1, TI_neighbors, patch_neighbors, 
            ...
            if ( ncoarse>start_score_based_dist )       // use pixel-based distance calculation
            {
                ...
                HD_DEV_Finder_Continuous hd_dev_finder( simul_grid_, neighbors, patcher, data_weight_ );

                if ( use_soft_field_ )
                {
                    Window_neighborhood* st_neighbors = simul_grid_->window_neighborhood( *window_geom_ );

                    ST_DEV_Finder_Continuous st_dev_finder( simul_grid_, st_neighbors, softDataProps_[0], data_weight_ );

                    status = sequential_patch_simulation_continuous_2(
                                                            path_begin, 
                                                            path_end,
                                                            proto_list3_[ proto_loc ],             
                                                            *continuous_sampler, 
                                                            hd_dev_finder,  
                                                            st_dev_finder,
                                                            patcher,
                ...
                {
                    status = sequential_patch_simulation_continuous(
                                                            path_begin, 
                                                            path_end,
                                                            proto_list3_[ proto_loc ],            
                                                            *continuous_sampler, 
            ...
            {
                DEV_Finder_Continuous_Score_based  hd_dev_score_finder( simul_grid_, neighbors, 
                                                                            my_filters_,  pattern_mapping_, max_value_, min_value_ );

                status = sequential_patch_simulation_continuous_score_based(
                                                        path_begin, 
                                                        path_end,
                                                        proto_list3_[ proto_loc ], 
                                                        *continuous_sampler, 
        ...
        else    // continuous + partition with k-mean
        {
            if ( nreal == 1 && first_run )
            {
                create_prototypelist( proto_list4_, nreal, ncoarse, 1, TI_neighbors, patch_neighbors, 
                                                                    cur_score, cmin_replicates_[ncoarse-1]);
            }
        ...
        // honor histogram of the training image
        if ( transcon_data_==1 && ncoarse == 2 )
        {
            GsTLcout << "Post-processing simulation on the penultimate coarse grid " << gstlIO::end;

            // build source cdf
            Non_param_cdf<> source_cdf;

            vector<float> sim_data;
            get_property_vector( prop, sim_data, true );

            build_cdf( sim_data.begin(), sim_data.end(), source_cdf, includes_max_ );

            // transform the property histogram towards training image histogram
            trans_prop_cdf( prop, source_cdf );
            
            if ( is_view_intermediate_ )    // record the transformed result
            {
                if ( use_score_dist_ )
                    record_intermediate_sim( prop, ncoarse, "_dual_trans" );
                else
                    record_intermediate_sim( prop, ncoarse, "_trans" );
            }
    ...
    if ( continuous_sampler != 0 ) delete continuous_sampler;
...
/*
 * function to store the data value from prop into a vector data
 */
void Filtersim_Cont::get_property_vector( GsTLGridProperty* prop, vector<float>& data, bool use_region )
{
    if ( use_region )
    {  // for simulations
        for (int i=0; i<prop->size(); i++)
        {
            if ( prop->is_informed(i) && region_[i] )
                data.push_back( prop->get_value(i) );
        }
    }
    else
    {  // for training image
...
/*
 * function to transform an input prop to honor the target histogram of training image
 */
void Filtersim_Cont::trans_prop_cdf( GsTLGridProperty* prop, Non_param_cdf<>& source_cdf )
{
    for (int i=0; i<prop->size(); i++)
    {
        if ( prop->is_informed(i) && region_[i] )
        {
            double P = source_cdf.prob( prop->get_value(i) );
            float val = target_cdf_.inverse(P);
 ...
 */
void Filtersim_Cont::record_intermediate_sim( GsTLGridProperty* prop, int ncoarse, string dual )
{
 ...
 */
void Filtersim_Cont::fillin_with_dual_template( GsTLGridProperty* prop, 
...
void 
Filtersim_Cont::create_prototypelist( vector<CPrototype>& proto_list,
                                                     ...
                                                     int cmin_replicates_ )
{
    ...
    CPrototype  proto( training_image_, TI_neighbors, patch_neighbors, cur_score, 
    ...
    proto_list[ nb_multigrids_-ncoarse ].create_prototype_list();

    // record the prototype indicator if needed
    if ( is_view_indicator_==1 && nreal==1 && ncoarse==1 )
        proto_list[ nb_multigrids_-ncoarse ].create_prototype_indicator();
}
