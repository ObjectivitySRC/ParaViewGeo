...
#ifndef _Filtersim_Cate_H__
#define _Filtersim_Cate_H__
...
// categorical+cross
typedef InitializePrototypeList<CrossPartition, Cate_Prototype> Initializer1;
typedef SplitPrototype<DefaultSplitter, Cate_Prototype> Splitter1;
typedef PrototypeList< Cate_Prototype, Initializer1, Splitter1, Distance > PrototypeListType1;

// categorical+kmean
typedef InitializePrototypeList<Prototype_kmeans, Cate_Prototype > Initializer2;
typedef SplitPrototype<Prototype_kmeans, Cate_Prototype> Splitter2;
typedef PrototypeList< Cate_Prototype, Initializer2, Splitter2, Distance > PrototypeListType2;


// filtersim class
class GEOSTAT_DECL Filtersim_Cate: public Geostat_algo 
{
public:
	Filtersim_Cate();
	~Filtersim_Cate();
	...
	virtual std::string name() const { return "filtersim_cate"; }
...
private:
    ...
    template <class CPrototype > void  create_prototypelist( CPrototype& proto_list, int nreal, int ncoarse, int nb_facies,
    ...
    bool get_marginal_cdf(std::string marginal_cpdf_str, Error_messages_handler* error_mesgs);
    bool get_marginal_cdf();
    bool get_current_cdf( GsTLGridProperty*prop );
    ...
    void get_target_pdf_factor( vector<float>& target_pdf_factor );
    void post_processing_realization( GsTLGridProperty*prop );
...
private:
    ...
    // honor TI proportions on the penultimate grid
    int transcon_data_;
